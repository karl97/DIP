...
% 2.2 a) going to derive my method on fewer images before applying on the
% whole set
...
%shoot
for i=1:length(images)
    got_ind(i)=images{i}.farm=="got";
    lan_ind(i)=images{i}.farm=="lan";
    mul_ind(i)=images{i}.farm=="mul";
...
d=mean(reshape([got.average_RGB],[3 length([got.average_RGB])/3])');
a2=mean([lan.average_area]);
b2=mean([lan.average_major_axis]);
c2=mean([lan.average_minor_axis]);
d2=mean(reshape([lan.average_RGB],[3 length([lan.average_RGB])/3])');
a3=mean([mul.average_area]);
b3=mean([mul.average_major_axis]);
c3=mean([mul.average_minor_axis]);
d3=mean(reshape([mul.average_RGB],[3 length([mul.average_RGB])/3])');

subplot(2,3,1);bar(categorical({'area','major axis','minor axis','R'}),[a,a2,a3;b,b2,b3;c,c2,c3;d(1),d2(1),d3(1)]);
...
subplot(2,3,2);bar(categorical({'area','major axis','minor axis'}),[a,a2;b,b2;c,c2]);
...
subplot(2,3,3);bar(categorical({'area','major axis','minor axis'}),[a,a2,a3;b,b2,b3;c,c2,c3]);
title('Type mean');legend('fat','bel','symp');
...
subplot(2,3,4);bar(categorical({'area','major axis','minor axis'}),[a,a2,a3;b,b2,b3;c,c2,c3]);
...
subplot(2,3,5);bar(categorical({'area','major axis','minor axis'}),[a,a2;b,b2;c,c2]);
...
subplot(2,3,6);bar(categorical({'area','major axis','minor axis'}),[a,a2,a3;b,b2,b3;c,c2,c3]);
title('Type std');legend('fat','bel','symp');
...
title('Type mean');legend('R/H','G/S','B/V');
...
title('Type std');legend('R/H','G/S','B/V');
